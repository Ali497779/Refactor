...
class BookingRepository extends BaseRepository
{
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if (isset($requestdata['filter_timetype']) && ($requestdata['filter_timetype'] == "created" || $requestdata['filter_timetype'] == "due")) {
                ...
                if (isset($requestdata['from']) && $requestdata['from'] != "") {
                    ...
                    $allJobs->where('jobs.' . $requestdata['filter_timetype'] . '_at', '>=', $from);
                }
                if (isset($requestdata['to']) && $requestdata['to'] != "") {
                    $to = Carbon::parse($requestdata["to"])->endOfDay();
                    $allJobs->where('jobs.' . $requestdata['filter_timetype'] . '_at', '<=', $to);
                }
                $allJobs->orderBy('jobs.' . $requestdata['filter_timetype'] . '_at', 'desc');
            }
            ...
            $allJobs = $allJobs->select('jobs.*', 'languages.language')->paginate(15);
        }
        // return ['allJobs' => $allJobs, 'languages' => $languages, 'all_customers' => $all_customers, 'all_translators' => $all_translators, 'requestdata' => $requestdata];
    ...
    public function reopen($request)
    {
        ...
        Translator::where('job_id', $jobid)->whereNull('cancel_at')->update(['cancel_at' => $currentTime]);
